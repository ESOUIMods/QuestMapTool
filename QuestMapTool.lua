...
function QuestMapTool.getSubzone()
  ...
  local function IsWorldByMapIndex()
    local mapIndex = GetCurrentMapIndex()
    local isWorld = (mapIndex == 1 or mapIndex == 24 or mapIndex == 40)
    if mapIndex and isWorld then
      return true
    end
    return false
  end

  local function IsWorldByMapId()
    local mapId = GetCurrentMapId()
    local isWorld = (mapId == 27 or mapId == 439 or mapId == 1782)
    if mapId and isWorld then
      return true
    end
    return false
  end
  ...
  if IsWorldByMapId() or IsWorldByMapIndex() then
  ...
  --[[todo check if world]]--
  if IsWorldByMapId() or IsWorldByMapIndex() then
    dm("Can't measure the workd maps")
